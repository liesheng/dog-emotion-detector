...
def avg_distortion_c_list(c_list, data, size=None):
    ...
    """
    size = size or len(data)
    #print("in avg_distortion_c_list, c_list.len:", len(c_list), " data.len:", len(data), " size:", size)
    #print("in avg_distortion_c_list, c_list[0].len:", len(c_list[0]), " data.len[0]:", len(data[0]), " size:", size)
    return reduce(lambda s, d:  s + d / size,
...
def euclid_squared(a, b):
    #print ("euclid_squared for"+str(a)+", "+str(b))
